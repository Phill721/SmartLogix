  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 select-none relative font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* ── Formulario ── */}
        <div className={`lg:col-span-2 p-6 sm:p-8 rounded-3xl border transition-all ${modoEdicion
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs font-bold text-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" disabled value={modoEdicion ? 'Gestionado en Bodega' : form.stock}
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="uppercase tracking-wider font-mono text-[11px] text-slate-500">
                  Fotos del Producto <span className="ml-2 text-emerald-600 font-bold">({form.imagenes.length})</span>
                </label>
                <button type="button" onClick={agregarImagen} className="text-[10px] font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-2.5 py-1 rounded-lg transition-all cursor-pointer">
                  + Agregar imagen
                </button>
              </div>
              <div className="space-y-2">
                {form.imagenes.map((url, i) => (
                  <div key={i} className="flex gap-2 items-center group">
                    ...
                    <input type="url" value={url} onChange={(e) => handleImagenChange(i, e.target.value)}
                    ...
                    {form.imagenes.length > 1 && (
                      <button type="button" onClick={() => eliminarImagen(i)}
                        className="text-slate-300 hover:text-rose-500 transition-colors text-sm px-1 shrink-0 cursor-pointer">✕</button>
                    )}
                  </div>
                ))}
              </div>
              <p className="text-[10px] font-mono text-slate-400 mt-1.5">La primera imagen será la foto principal del producto.</p>
            </div>

            <div>
              <label className="block uppercase tracking-wider mb-1 font-mono text-[11px] text-slate-500">Descripción</label>
              <textarea name="descripcion" rows="3" required value={form.descripcion} onChange={handleChange}
            ...
            <div className="flex gap-3 pt-4">
              <button type="submit" disabled={cargando}
                className={`flex-1 text-white py-4 rounded-xl font-black uppercase tracking-widest drop-shadow-md hover:opacity-95 active:scale-[0.99] transition-all cursor-pointer text-xs ${modoEdicion ? 'bg-amber-600 hover:bg-amber-700' : 'bg-[#1E3859]'
                  }`}>
              ...
              {modoEdicion && (
                <button type="button" disabled={cargando}
                  ...
                  className="px-5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-xl font-bold transition-all cursor-pointer text-base" title="Dar de baja">
                  🗑️
                </button>
              )}
            </div>
          ...
          <div>
            <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-4">
              ...
              <span className="bg-emerald-400/20 text-emerald-300 font-mono text-[10px] px-2.5 py-1 rounded-full border border-emerald-400/30 font-bold">
                {productsFiltrados.length}/{products.length} SKUs
              </span>
            </div>
            ...
            <input
              ...
              className="w-full bg-white/10 border border-white/10 focus:border-white/30 text-white placeholder-slate-400 font-mono text-[11px] rounded-xl px-3 py-2.5 outline-none transition-all mb-4"
            />
            ...
            <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
              {productsFiltrados.length === 0 ? (
                ...
                <div
                  ...
                  className={`p-3.5 rounded-2xl border flex items-center justify-between gap-3 text-xs transition-all cursor-pointer group ${form.sku === p.sku && modoEdicion
                ...
                >
                  <div className="overflow-hidden">
                    ...
                    {p.imagenes?.length > 1 && (
                      <span className="text-[9px] font-mono text-slate-400 mt-0.5 block">📷 {p.imagenes.length} fotos</span>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-md w-full drop-shadow-[0_25px_50px_rgba(0,0,0,0.25)] text-slate-800">
            ...
            <div className="flex gap-3">
              <button type="button" disabled={cargando} onClick={() => setModalEliminar(null)}
              ...
              <button type="button" disabled={cargando} onClick={ejecutarEliminacion}
